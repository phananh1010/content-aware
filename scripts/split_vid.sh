...
for dirpath in ${ROOT_SCALED}*; do
    ...
    for template in "${arr_vid[@]}"; do
        ...
        for filepath in ${dirpath}/*${template}*.mp4; do
            ...
            neg_pattern=_${template}.mp4
            image_template=${filename/${neg_pattern}/}
            
            #then, split the video
            echo "splitting vid: $filepath into ${dir_template}/${image_template}%06d.jpg"
            ffmpeg -i $filepath ${dir_template}/${image_template}%06d.jpg -hide_banner
        done
    done
done
